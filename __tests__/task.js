...
describe("basic HTML structure", () => {
  test("Document type has been declared", () => {
    ...
    expect(htmlDoc).toMatch(/<.*!DOCTYPE.*html.*>/gim);
  });
  test("HTML element has been declared", () => {
    ...
    expect(htmlDoc).toMatch(/<html.*>/gim);
  });
  test("Head element has been declared", () => {
    ...
    expect(htmlDoc).toMatch(/<head.*>/gim);
  });
  test("Body element has been declared", () => {
    ...
    expect(htmlDoc).toMatch(/<body.*>/gim);
  });
});
describe("Form fields", () => {
  test("Form has been declared", () => {
    ...
    expect(htmlDoc).toMatch(/<form.*>/gim);
  }),
  ...
  test("Form has at least one date field", async () => {
    ...
    const dateFields = await page.$$eval("input", (els) =>
      els.filter((el) => el.type.includes("date"))
    );
...
describe("Styling Forms", () => {
  ...
  test("The form should centered on the page", async () => {
    ...
    expect(formPosition).toBeGreaterThan(
      pageSize.width / 2 - formWidth / 2 - 10
    ) &&
      expect(formPosition).toBeLessThan(
        pageSize.width / 2 + formWidth / 2 + 10
      ); // The student has 10px margin on each side of the form
  });
